...
firefox_detection=true
chromium_detection=true
...
maybe_delay_screensaver () {
    if we_are_fullscreen; then
		...
		$verbose && echo "not detected fullscreen"
	fi
}
...
app_is_running () {
	...
	$verbose && echo "	active win seems to be VLC"
        return 0
    fi
	...
	$verbose && echo "	active win seems to be Parole"
        return 0
    fi

	if $firefox_detection &&
	[[ $active_win_title = *firefox* || $active_win_title = *Firefox* ]] &&
	pgrep firefox &>/dev/null; then
	$verbose && echo "	active win seems to be Firefox"
	    return 0
	fi
	
	if $firefox_detection &&
	[[ $active_win_title = *chromium* || $active_win_title = *Chromium* ]] &&
	pgrep chromium-browse &>/dev/null; then
	$verbose && echo "	active win seems to be Chromium"
	    return 0
	fi

	: ' # the firefox and chromium detector below is somewhat broken.
		# So i just detect firefox and chromium, no flash-player.
	...
	# TODO: the hardcoded path probably does not always work
	$verbose && echo "	active win seems to be Chromium"
        return 0
    fi
	'
